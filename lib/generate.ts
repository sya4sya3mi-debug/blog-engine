...
/** パース結果からGeneratedArticleを組み立てる（構造化データ埋め込み + コンプライアンス修正） */
function buildGeneratedArticle(parsed: ParsedArticle, keyword: string, themeLabel: string): GeneratedArticle {
  ...
  // 構造化データをHTMLに埋め込む（Article + FAQ のみ。Product/ItemList はアフィリエイトブログでは効果薄のため除外）
  let schemaHtml = buildArticleSchema(parsed.title, parsed.metaDescription, focusKeyword);
  schemaHtml += buildFaqSchema(faq);

  const htmlContent = complianceFixed + schemaHtml;
...
- 医療行為に関する断定表現は避ける（「○○の効果がある」→「○○の効果が期待できるとされています」）
- 薬機法に抵触する表現は使わない
...
/**
 * アイキャッチ画像URLを記事に反映する（構造化データ + OGP用）
 * WordPress投稿後に呼び出し、Article JSON-LDの image フィールドを更新
 */
export function injectEyecatchIntoArticle(article: GeneratedArticle, eyecatchUrl: string): GeneratedArticle {
  ...
  // Article JSON-LD に image フィールドを追加/更新
  const articleSchemaRegex = /<script type="application\/ld\+json">({"@context":"https:\/\/schema\.org","@type":"Article"[^<]*})<\/script>/;
  const match = article.htmlContent.match(articleSchemaRegex);
  if (match) {
    try {
      const schema = JSON.parse(match[1]);
      schema.image = eyecatchUrl;
...
export async function generatePersonalReviewArticle(
  ...
  const reviewSchema = {
    ...
    itemReviewed: {
      ...
      ...(review.price ? { offers: { "@type": "Offer", price: review.price, priceCurrency: "JPY" } } : {}),
    },
    reviewRating: {
      "@type": "Rating",
    ...
    author: { "@type": "Person", name: "みお" },
    reviewBody: `${review.skinType}の筆者が${review.usagePeriod}使用。${review.goodPoints.slice(0, 80)}`,
  };
