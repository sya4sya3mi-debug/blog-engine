...
  item: any; editContent: string; setEditContent: (v: string) => void;
...
      <div style={{ background: S.card, border: `1px solid ${item.siteColor}44`, borderRadius: isMobile ? "16px 16px 0 0" : 16, width: "100%", maxWidth: isMobile ? "100%" : 820, maxHeight: isMobile ? "92vh" : "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: isMobile ? "16px" : "18px 24px", borderBottom: `1px solid ${S.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ flex: 1, minWidth: 0, paddingRight: 12 }}>
            <div style={{ fontWeight: 800, fontSize: isMobile ? 14 : 16, lineHeight: 1.4, marginBottom: 4 }}>{item.title}</div>
            <div style={{ fontSize: 11, color: "#555570", display: "flex", gap: 12, flexWrap: "wrap" as const }}>
              <span>{item.site}</span><span>投稿予定：{item.scheduledFor}</span><span style={{ color: "#00C89688" }}>期限なし</span>
            </div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#555570", fontSize: 26, cursor: "pointer", lineHeight: 1, padding: "0 0 0 8px", flexShrink: 0 }}>×</button>
        </div>
...
            <textarea value={editContent} onChange={e => setEditContent(e.target.value)} style={{ width: "100%", minHeight: isMobile ? 200 : 300, background: "#14141F", border: "1.5px solid #2A2A3C", borderRadius: 10, padding: "12px 14px", color: "#E8E8F0", fontSize: 13, lineHeight: 1.8, outline: "none", resize: "vertical" as const, boxSizing: "border-box" as const, fontFamily: "inherit" }} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "#888899", fontWeight: 600, display: "block", marginBottom: 8 }}>差し戻しコメント（差し戻す場合のみ）</label>
            <input value={revisionComment} onChange={e => setRevisionComment(e.target.value)} placeholder="修正内容を入力..." style={{ width: "100%", background: "#14141F", border: "1.5px solid #2A2A3C", borderRadius: 10, padding: "11px 14px", color: "#E8E8F0", fontSize: 13, outline: "none", boxSizing: "border-box" as const }} />
          </div>
          {sendResult && (
...
              {sendResult === "success" ? "✓ メール通知を送信しました" : "✗ メール送信に失敗しました"}
            </div>
          )}
        </div>
...
          {!isMobile && <div style={{ fontSize: 12, color: "#555570", marginRight: "auto" }}>承認後：{item.scheduledFor} に自動投稿</div>}
...
          <button onClick={onApprove} disabled={sending} style={{ flex: isMobile ? 2 : "none" as any, padding: isMobile ? "12px" : "10px 24px", borderRadius: 10, border: "none", background: sending ? "#1A1A28" : `linear-gradient(135deg,${item.siteColor},${item.siteColor}BB)`, color: sending ? "#444455" : "#000", fontSize: 13, fontWeight: 800, cursor: sending ? "not-allowed" : "pointer" }}>
...
// ── 生成モーダル ────────────────────────────────────────────────────
function GenModal({ show, generating, siteName, siteColor, keyword, streamText, streamRef, onClose, onGoReview }: {
  show: boolean; generating: boolean; siteName: string; siteColor: string;
  keyword: string; streamText: string; streamRef: React.RefObject<HTMLDivElement>;
  onClose: () => void; onGoReview: () => void;
}) {
  if (!show) return null;
  return (
...
        <div>
          <div style={{ fontWeight: 800, fontSize: 14, color: generating ? siteColor : "#00C896" }}>{generating ? "✦ 生成中..." : "✓ 生成完了"}</div>
          <div style={{ fontSize: 11, color: "#555570", marginTop: 2 }}>{siteName} · {keyword}</div>
        </div>
        {!generating && (
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={onClose} style={{ padding: "7px 14px", borderRadius: 8, border: `1px solid ${S.border}`, background: "transparent", color: "#888899", fontSize: 12, cursor: "pointer" }}>閉じる</button>
            <button onClick={onGoReview} style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: `linear-gradient(135deg,${siteColor},${siteColor}BB)`, color: "#000", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>レビューへ →</button>
          </div>
        )}
      </div>
      <div ref={streamRef} style={{ flex: 1, overflow: "auto", padding: "16px", fontSize: 13, lineHeight: 1.9, color: "#C8C8D8", whiteSpace: "pre-wrap" }}>
        {streamText}{generating && <span style={{ color: siteColor }}>▋</span>}
      </div>
    </div>
  );
}
...
// ── キューアイテム ────────────────────────────────────────────────
function QueueItem({ item, onReview, mobile }: { item: any; onReview: (item: any) => void; mobile: boolean }) {
  const st = STATUS_MAP[item.status] ?? STATUS_MAP.published;
  return (
...
          </div>
          <div style={{ fontSize: mobile ? 13 : 14, fontWeight: 700, lineHeight: 1.4, marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: mobile ? "normal" : "nowrap" as const }}>{item.title}</div>
          <div style={{ fontSize: 11, color: "#555570" }}>投稿予定：{item.scheduledFor} <span style={{ color: "#00C89688" }}>· 期限なし</span></div>
...
        </div>
        {!mobile && item.status !== "published" && item.status !== "approved" && (
          <button onClick={() => onReview(item)} style={{ padding: "8px 18px", borderRadius: 8, border: `1px solid ${item.siteColor}55`, background: "transparent", color: item.siteColor, fontSize: 13, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>{item.status === "revision" ? "再レビュー" : "レビューする"}</button>
        )}
...
      </div>
      {mobile && item.status !== "published" && item.status !== "approved" && (
        <button onClick={() => onReview(item)} style={{ width: "100%", marginTop: 10, padding: "10px", borderRadius: 8, border: `1px solid ${item.siteColor}55`, background: "transparent", color: item.siteColor, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{item.status === "revision" ? "再レビューする" : "レビューする"}</button>
      )}
...
// ── 記事生成フォーム ──────────────────────────────────────────────
function GenerateForm({ selectedSite, setSelectedSite, keyword, setKeyword, affiliate, setAffiliate, scheduleDate, setScheduleDate, scheduleTime, setScheduleTime, generating, onGenerate, mobile }: any) {
  return (
...
        <div style={{ display: "flex", flexDirection: mobile ? "column" : "row", gap: mobile ? 8 : 10 }}>
          {SITES.map(site => <button key={site.id} onClick={() => setSelectedSite(site)} style={{ padding: mobile ? "10px 14px" : "8px 16px", borderRadius: 8, border: `1.5px solid ${selectedSite.id === site.id ? site.color : "#2A2A3C"}`, background: selectedSite.id === site.id ? site.color + "18" : "transparent", color: selectedSite.id === site.id ? site.color : "#666677", fontSize: 13, fontWeight: selectedSite.id === site.id ? 700 : 400, cursor: "pointer", textAlign: "left" as const }}>{site.name}{mobile && <span style={{ fontSize: 11, opacity: 0.6 }}> — {site.domain}</span>}</button>)}
        </div>
      </div>
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 12, color: "#888899", display: "block", marginBottom: 8, fontWeight: 600 }}>ターゲットキーワード</label>
        <input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="例：ワイヤレスイヤホン おすすめ" style={{ width: "100%", background: "#14141F", border: "1.5px solid #2A2A3C", borderRadius: 10, padding: "12px 14px", color: "#E8E8F0", fontSize: 14, outline: "none", boxSizing: "border-box" as const }} onFocus={e => e.target.style.borderColor = selectedSite.color} onBlur={e => e.target.style.borderColor = "#2A2A3C"} />
      </div>
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 12, color: "#888899", display: "block", marginBottom: 8, fontWeight: 600 }}>アフィリエイト</label>
        <div style={{ display: "flex", gap: 8 }}>{AFFILIATES.map(af => <button key={af.id} onClick={() => setAffiliate(af.id)} style={{ flex: 1, padding: "9px 8px", borderRadius: 8, border: `1.5px solid ${affiliate === af.id ? af.color : "#2A2A3C"}`, background: affiliate === af.id ? af.color + "18" : "transparent", color: affiliate === af.id ? af.color : "#666677", fontSize: 12, fontWeight: affiliate === af.id ? 700 : 400, cursor: "pointer" }}>{af.name}</button>)}</div>
      </div>
      <div style={{ marginBottom: 20, padding: "12px 14px", background: "#14141F", borderRadius: 10, border: `1px solid ${S.border}` }}>
        <div style={{ fontSize: 11, color: "#888899", fontWeight: 600, marginBottom: 8 }}>◷ 投稿スケジュール（承認後に有効）</div>
        <div style={{ display: "flex", gap: 8 }}>
          <input type="date" value={scheduleDate} onChange={e => setScheduleDate(e.target.value)} style={{ flex: 1, background: "#0F0F1A", border: `1px solid ${S.border}`, borderRadius: 8, padding: "8px 10px", color: "#E8E8F0", fontSize: 12, outline: "none", minWidth: 0 }} />
          <input type="time" value={scheduleTime} onChange={e => setScheduleTime(e.target.value)} style={{ width: 90, background: "#0F0F1A", border: `1px solid ${S.border}`, borderRadius: 8, padding: "8px 10px", color: "#E8E8F0", fontSize: 12, outline: "none" }} />
        </div>
        <div style={{ fontSize: 11, color: "#555570", marginTop: 6 }}>※ 承認した後にのみ、この日時に投稿されます</div>
      </div>
      <button onClick={onGenerate} disabled={generating || !keyword.trim()} style={{ width: "100%", background: generating || !keyword.trim() ? "#1A1A28" : `linear-gradient(135deg,${selectedSite.color},${selectedSite.color}BB)`, border: "none", borderRadius: 10, padding: "14px", color: generating || !keyword.trim() ? "#444455" : "#000", fontWeight: 800, fontSize: 14, cursor: generating || !keyword.trim() ? "not-allowed" : "pointer" }}>
...
    try {
      const res = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ keyword, siteTheme, afName }) });
...
        ✓ 生成後はレビューキューに追加されます<br />✓ 承認するまで投稿されません（期限なし）<br />✓ kopperian432432@gmail.com にメール通知
      </div>
    </div>
  );
...
      <div style={{ display: isMobile ? "flex" : "grid", flexDirection: isMobile ? "column" : undefined, gridTemplateColumns: isMobile ? undefined : "repeat(3,1fr)", gap: isMobile ? 10 : 14 }}>
        {SITES.map(site => (
          <div key={site.id} style={{ background: S.card, border: `1px solid ${site.color}33`, borderRadius: isMobile ? 12 : 14, padding: isMobile ? "14px 16px" : 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ fontWeight: 700, fontSize: isMobile ? 14 : 15, marginBottom: 4 }}>{site.name}</div><div style={{ fontSize: 11, color: "#555570" }}>{site.domain}</div></div>
            <div style={{ display: "flex", gap: 16, textAlign: "right" as const }}>
              <div><div style={{ fontSize: isMobile ? 18 : 20, fontWeight: 800, color: site.color }}>{site.articles}</div><div style={{ fontSize: 10, color: "#555570" }}>記事</div></div>
              <div><div style={{ fontSize: isMobile ? 18 : 20, fontWeight: 800, color: site.color }}>¥{site.revenue.toLocaleString()}</div><div style={{ fontSize: 10, color: "#555570" }}>収益</div></div>
            </div>
          </div>
        ))}
      </div>
...
          <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{a.title}</div><div style={{ fontSize: 11, color: "#555570", marginTop: 2 }}>{a.site} · {a.date}</div></div>
          <div style={{ display: "flex", gap: 20 }}><div style={{ textAlign: "right" as const }}><div style={{ fontSize: 13, fontWeight: 700 }}>{a.views.toLocaleString()}</div><div style={{ fontSize: 10, color: "#555570" }}>PV</div></div><div style={{ textAlign: "right" as const }}><div style={{ fontSize: 13, fontWeight: 700 }}>{a.clicks}</div><div style={{ fontSize: 10, color: "#555570" }}>クリック</div></div></div>
        </div>
      ))}
    </div>
  );
...
            <div style={{ width: isMobile ? 8 : 10, height: isMobile ? 8 : 10, borderRadius: "50%", background: af.color }} />
            <div><div style={{ fontWeight: 700, fontSize: isMobile ? 13 : 15 }}>{af.name}</div><div style={{ fontSize: 11, color: "#555570", marginTop: 2 }}>自動挿入ON</div></div>
          </div>
...
          <div style={{ fontSize: 18, fontWeight: 800, background: "linear-gradient(135deg,#00D4FF,#00C896)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>● BlogEngine</div>
...
          {SITES.map(site => <div key={site.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: site.status === "active" ? site.color : "#333344" }} /><span style={{ fontSize: 12, color: "#666677" }}>{site.name}</span></div>)}
        </div>
      </aside>
...
          <div>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800 }}>{activeTab === "overview" ? "ダッシュボード" : activeTab === "review" ? "レビュー待ち" : activeTab === "generate" ? "AI記事生成" : activeTab === "articles" ? "記事一覧" : "アフィリエイト設定"}</h1>
...
        {[{ id: "overview", label: "ホーム", icon: "⬡" },{ id: "review", label: "レビュー", icon: "◈" },{ id: "generate", label: "生成", icon: "✦" },{ id: "articles", label: "記事", icon: "≡" },{ id: "affiliate", label: "AF", icon: "◎" }].map(item => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "10px 4px", border: "none", background: "transparent", cursor: "pointer", position: "relative" }}>
            {item.id === "review" && badgeCount > 0 && <span style={{ position: "absolute", top: 6, right: "50%", transform: "translateX(10px)", background: "#FF6B6B", color: "#fff", fontSize: 9, fontWeight: 800, borderRadius: 8, padding: "1px 5px" }}>{badgeCount}</span>}
            <span style={{ fontSize: 18, color: activeTab === item.id ? "#00D4FF" : "#555570" }}>{item.icon}</span>
            <span style={{ fontSize: 10, color: activeTab === item.id ? "#00D4FF" : "#555570", fontWeight: activeTab === item.id ? 700 : 400 }}>{item.label}</span>
          </button>
        ))}
      </nav>
      <GenModal show={showGenModal} generating={generating} siteName={selectedSite.name} siteColor={selectedSite.color} keyword={keyword} streamText={streamText} streamRef={streamRef} onClose={() => setShowGenModal(false)} onGoReview={() => { setShowGenModal(false); setKeyword(""); setActiveTab("review"); }} />
