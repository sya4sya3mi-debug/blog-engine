...
          {d.activeTab === "overview" && (<div><div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 16, marginBottom: 28 }}>{[{ label: "総収益", value: `¥${totalRevenue.toLocaleString()}`, color: "#00C896" },{ label: "公開記事数", value: SITES.reduce((s, x) => s + x.articles, 0), color: "#00D4FF" },{ label: "レビュー待ち", value: d.pendingCount, color: "#FFB347" },{ label: "差し戻し中", value: d.revisionCount, color: "#FF6B6B" }].map((kpi, i) => (<div key={i} style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: 14, padding: "20px 22px", position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: kpi.color }} /><div style={{ fontSize: 11, color: "#555570", marginBottom: 8, textTransform: "uppercase" as const, letterSpacing: "0.5px" }}>{kpi.label}</div><div style={{ fontSize: 28, fontWeight: 800, color: kpi.color }}>{kpi.value}</div></div>))}</div><div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 14 }}>{SITES.map(site => (<div key={site.id} style={{ background: S.card, border: `1px solid ${site.color}33`, borderRadius: 14, padding: 20, minWidth: 0 }}><div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4, whiteSpace: "nowrap" as const, overflow: "hidden", textOverflow: "ellipsis" }}>{site.name}</div><div style={{ fontSize: 11, color: "#555570", marginBottom: 14, whiteSpace: "nowrap" as const, overflow: "hidden", textOverflow: "ellipsis" }}>{site.domain}</div><div style={{ display: "flex", gap: 20 }}><div><div style={{ fontSize: 20, fontWeight: 800, color: site.color }}>{site.articles}</div><div style={{ fontSize: 10, color: "#555570" }}>記事</div></div><div><div style={{ fontSize: 20, fontWeight: 800, color: site.color }}>¥{site.revenue.toLocaleString()}</div><div style={{ fontSize: 10, color: "#555570" }}>収益</div></div></div></div>))}</div></div>)}
...
        {NAV.map(item => (<button key={item.id} onClick={() => d.setActiveTab(item.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "10px 4px", border: "none", background: "transparent", cursor: "pointer", position: "relative" }}>{item.id === "review
