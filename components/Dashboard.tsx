...
          <div style={{ flex: 1, minWidth: 0, paddingRight: 12 }}><div style={{ fontWeight: 800, fontSize: isMobile ? 14 : 16, lineHeight: 1.4, marginBottom: 4 }}>{item.title}</div><div style={{ fontSize: 11, color: "#555570", display: "flex", gap: 12, flexWrap: "wrap" as const }}><span>{item.site}</span><span>投稿予定：{item.scheduled_for}</span><span style={{ color: "#00C89688" }}>期限なし</span></div></div>
...
function GenModal({ show, generating, siteName, siteColor, keyword, streamText, streamRef, onClose, onGoReview }: any) {
  if (!show) return null;
  return (
...
        <div><div style={{ fontWeight: 800, fontSize: 14, color: generating ? siteColor : "#00C896" }}>{generating ? "✦ 生成中..." : "✓ 生成完了"}</div><div style={{ fontSize: 11, color: "#555570", marginTop: 2 }}>{siteName} · {keyword}</div></div>
        {!generating && (<div style={{ display: "flex", gap: 8 }}><button onClick={onClose} style={{ padding: "7px 14px", borderRadius: 8, border: `1px solid ${S.border}`, background: "transparent", color: "#888899", fontSize: 12, cursor: "pointer" }}>閉じる</button><button onClick={onGoReview} style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: `linear-gradient(135deg,${siteColor},${siteColor}BB)`, color: "#000", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>レビューへ →</button></div>)}
      </div>
      <div ref={streamRef} style={{ flex: 1, overflow: "auto", padding: "16px", fontSize: 13, lineHeight: 1.9, color: "#C8C8D8", whiteSpace: "pre-wrap" }}>{streamText}{generating && <span style={{ color: siteColor }}>▋</span>}</div>
    </div>
  );
}

function QueueItem({ item, onReview, mobile }: { item: Article; onReview: (item: Article) => void; mobile: boolean }) {
  const st = STATUS_MAP[item.status] ?? STATUS_MAP.published;
  return (
    <div style={{ background: S.card, border: `1px solid ${item.status === "pending" ? item.site_color + "33" : item.status === "revision" ? "#FF6B6B33" : S.border}`, borderRadius: 14, padding: mobile ? "14px 16px" : "18px 22px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{ width: 4, height: 48, borderRadius: 2, background: item.status === "pending" ? item.site_color : item.status === "revision" ? "#FF6B6B" : "#333344", flexShrink: 0 }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" as const }}><span style={{ fontSize: 10, padding: "3px 8px", borderRadius: 4, fontWeight: 700, background: st.bg, color: st.color }}>{st.label}</span><span style={{ fontSize: 11, color: "#555570" }}>{item.site}</span></div>
...
          <div style={{ fontSize: 11, color: "#555570" }}>投稿予定：{item.scheduled_for} <span style={{ color: "#00C89688" }}>· 期限なし</span></div>
...
  const [reviewItem, setReviewItem] = useState<Article | null>(null);
...
          const newArticle = await saveArticle({ site: selectedSite.name, site_color: selectedSite.color, title, keyword, generated_at: now, scheduled_for: `${scheduleDate} ${scheduleTime}`, status: "pending", content: text, comment: "" });
          if (newArticle) setQueue(prev => [newArticle, ...prev]);
          setStreamText(text);
